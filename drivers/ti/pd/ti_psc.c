...
static uint32_t psc_read(struct ti_device *dev, uint32_t reg)
{
	const struct ti_resource_mem *mem =
		(const struct ti_resource_mem *) ti_resource_get(dev,
							      TI_RESOURCE_MEM,
							      0U);

	if (mem == NULL || mem->addr == TI_RESOURCE_MEM_NONE)
		return 0U;
...
static int32_t psc_pre_init(struct ti_device *dev)
{
	...
	if (ti_psc_lookup((ti_psc_idx_t) psc->psc_idx) == NULL) {
		...
		mem = (const struct ti_resource_mem *) ti_resource_get(dev, TI_RESOURCE_MEM, 0U);
		if (mem == NULL || mem->addr == TI_RESOURCE_MEM_NONE) {
			return -EINVAL;
		}
