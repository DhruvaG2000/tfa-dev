...
static int32_t am62l_pwr_domain_on(u_register_t mpidr)
{
	int32_t core, ret;
	uint8_t proc_id;
	...
	if (core < 0) {
		...
		if (ret != 0) {
			...
			if (ret != 0) {
				ERROR("Request to set core boot address failed:"
				      "%d\n", ret);
				ret = PSCI_E_INTERN_FAIL;
			} else {
				...
				if (ret != 0) {
					ERROR("Request to clear boot configuration"
					      "failed: %d\n", ret);
					ret = PSCI_E_INTERN_FAIL;
				} else {
					/*
...
static void __dead2 am62l_pwr_domain_off_wfi(const psci_power_state_t *target_state)
{
	...
	if (((target_state)->pwr_domain_state[MPIDR_AFFLVL1]) != PLAT_MAX_OFF_STATE) {
